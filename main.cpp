...
void RenderGame(RenderWindow &game, int &score, List lst) {
	...
	//задання вертикальної синхронізації
	game.setVerticalSyncEnabled(true);

	//задання шрифту(з файлу)
	Font font;
	...
	//додавання текстур, фону (з файлу)
	...
	//Додавання візуалізації карти гри, фону, текстур та їх позицій. 
	...
	//цикл гри. пока гра запущена, вона завантажується і створюється подія. Потім зберігається розміщення у текстовий файл та гра закривається.
	while (isGame) {
		Event event;
		...
		//задання властивостей блоків(прямокутників)
		RectangleShape rectangle(Vector2f(400.f, 60.f));
		rectangle.move(0, 0);
		rectangle.setFillColor(Color(153, 204, 255, 225));

		//подія на нажаття кнопки. Розставляється текстури на позиції
...
//головний метод, який створює усі об'єкти гри та вікно гри, меню.
int main() {
